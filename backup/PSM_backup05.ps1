...
$scriptFiles = Get-ChildItem -Path $scriptDirectory -File -ErrorAction SilentlyContinue -WarningAction SilentlyContinue | Where-Object { $_.Name -match '^PSM\d+\.ps1$' }
...
foreach ($scriptFile in $scriptFiles) {
    ...
    if ($scriptVersion -gt $highestScriptVersion) {
        ...
        $jsonFilePath = Join-Path -Path $scriptDirectory -ChildPath ($scriptFile.Name -replace '\.ps1$', '.json')
